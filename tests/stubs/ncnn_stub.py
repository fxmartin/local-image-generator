#!/usr/bin/env python3
"""Stub `qwenimage-ncnn-vulkan`: echo argv, print fake progress, write a tiny PNG to `-o`.
...
def main(argv: list[str]) -> int:
    ...
    if os.environ.get("STUB_FAIL") == "1":
        print("qwenimage-ncnn-vulkan: stub failure requested (STUB_FAIL=1)", file=sys.stderr)
        return 2

    if "-o" not in argv or argv.index("-o") + 1 >= len(argv):
        print("qwenimage-ncnn-vulkan: missing -o <output path>", file=sys.stderr)
        return 1
    out = argv[argv.index("-o") + 1]

    for step in (1, 2, 3, 4):
        print(f"{step * 25}.00%", flush=True)
